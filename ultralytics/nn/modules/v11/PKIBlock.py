...
__all__ = ['PKINET_T', 'PKINET_S', 'PKINET_B', 'CAA', 'PKIBlock11']

class BaseModule(nn.Module):
    """A tiny replacement for mmengine.model.BaseModule.

    Supports: super().__init__(init_cfg), self.init_cfg, self.init_weights()
    """
    ...
    def init_weights(self):
        """Apply simple initialization if init_cfg is provided.
        If you don't need it, you can leave init_cfg=None everywhere.
        """
        cfg = self.init_cfg
        if not cfg:
            return
        ...
        # Common case in your code: Kaiming init for Conv2d
        if cfg_type == "kaiming":
            a = cfg.get("a", math.sqrt(5))
            mode = cfg.get("mode", "fan_in")
            nonlinearity = cfg.get("nonlinearity", "leaky_relu")
            distribution = cfg.get("distribution", "uniform")
            layer = cfg.get("layer", "Conv2d")

            for m in self.modules():
                if layer.lower() == "conv2d" and isinstance(m, nn.Conv2d):
        ...
        # Extend here if you have more init_cfg types later.


# -------------------------
# Init helpers (mmcv-free)
# -------------------------
def trunc_normal_init(m: nn.Module, std: float = 0.02, bias: float = 0.0):
...
def constant_init(m: nn.Module, val: float = 1.0, bias: float = 0.0):
...
def normal_init(m: nn.Module, mean: float = 0.0, std: float = 1.0, bias: float = 0.0):
    if hasattr(m, "weight") and m.weight is not None:
...
# -------------------------
# Norm builder + ConvModule (mmcv-free)
# -------------------------
def build_norm_layer(norm_cfg: Optional[dict], num_features: int) -> Tuple[str, nn.Module]:
    """Return (name, layer) similar to mmcv.build_norm_layer."""
    if norm_cfg is None:
        return "identity", nn.Identity()

    cfg = dict(norm_cfg)  # shallow copy
    norm_type = cfg.pop("type", "BN")

    if norm_type in ["BN", "BN2d", "BatchNorm2d"]:
        eps = cfg.pop("eps", 1e-3)
        momentum = cfg.pop("momentum", 0.03)
        layer = nn.BatchNorm2d(num_features, eps=eps, momentum=momentum, **cfg)
        return "bn", layer

    if norm_type in ["GN", "GroupNorm"]:
        # mmcv usually uses num_groups; default 32
        num_groups = cfg.pop("num_groups", 32)
        eps = cfg.pop("eps", 1e-5)
        layer = nn.GroupNorm(num_groups=num_groups, num_channels=num_features, eps=eps, **cfg)
        return "gn", layer

    if norm_type in ["LN", "LayerNorm"]:
        eps = cfg.pop("eps", 1e-5)
        # NOTE: LN for BCHW is tricky; your code uses LN in BHWC in ConvFFN already.
        # Here we provide a safe version for channels-only LN in BHWC usage.
...
def _build_act(act_cfg: Optional[dict]) -> nn.Module:
    if act_cfg is None:
        return nn.Identity()
    cfg = dict(act_cfg)
    act_type = cfg.pop("type", "SiLU")

    if act_type in ["SiLU", "Swish"]:
        return nn.SiLU(**cfg)
    if act_type in ["ReLU"]:
        return nn.ReLU(**cfg)
    if act_type in ["LeakyReLU"]:
        return nn.LeakyReLU(**cfg)
    if act_type in ["GELU"]:
        return nn.GELU(**cfg)
    if act_type in ["Sigmoid"]:
        return nn.Sigmoid()
    if act_type in ["Identity", "None"]:
        return nn.Identity()

    raise ValueError(f"Unsupported act type: {act_type}")


class ConvModule(nn.Module):
    """A lightweight replacement of mmcv.cnn.ConvModule.

    Conv2d -> (optional Norm) -> (optional Act)
    """
    def __init__(
        self,
        in_channels: int,
        ...
        # if norm is used, conv bias is usually False
        if bias is None:
        ...
        self.norm_name, self.norm = build_norm_layer(norm_cfg, out_channels) if norm_cfg else ("identity", nn.Identity())
        self.act = _build_act(act_cfg) if act_cfg else nn.Identity()
...
class PKIBlock11(nn.Module):
    """Ultralytics-YAML friendly wrapper.
    - c1: in channels (framework inferred)
    - c2: out channels (yaml arg)
    - n : repeats (yaml field)
    """
    def __init__(self, c1, c2, n=1,
                 with_caa=True, caa_kernel_size=11,
                 expansion=1.0, ffn_scale=4.0, ffn_kernel_size=3,
                 dropout_rate=0.0, drop_path_rate=0.0, layer_scale=1.0):
        super().__init__()

        # 通道不一致就用 1x1 对齐（更通用）
        self.align = nn.Identity() if c1 == c2 else nn.Conv2d(c1, c2, 1, 1, 0, bias=False)
        ...
        self.m = nn.Sequential(*[
            PKIBlock(
                ...
                out_channels=c2,
                with_caa=with_caa,
                caa_kernel_size=caa_kernel_size,
                expansion=expansion,
                ffn_scale=ffn_scale,
